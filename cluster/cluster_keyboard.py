...
class ClusterWindow(QMainWindow):
    def __init__(self):
        ...
        self.background_pixmap = QPixmap("/home/user/Ota/image/back.png")  # 배경 이미지 경로 설정
        ...
        self.logo_pixmap = QPixmap("/home/user/Ota/image/volk.png")  # 로고 이미지 경로 설정
    ...
    def keyPressEvent(self, event):
        if event.key() == Qt.Key_Escape:
            ...
            self.text_label.setStyleSheet('color:rgb(255,255,255)')
            self.update_logo("N")
        elif event.key() == Qt.Key_Up and self.text_label.text() != "P":
            self.current_speed = min(self.current_speed + 3, 300)  # 위 방향키로 엑셀
            if self.current_rpm < 200:
                self.current_rpm = min(self.current_rpm + 20, 200)
            else:
                self.current_rpm = min(self.current_rpm + 1, 300)
            self.update_speed()
        elif event.key() == Qt.Key_Down:
            self.current_speed = max(self.current_speed - 1, 0)  # 아래 방향키로 브레이크
            self.current_rpm = max(self.current_rpm + 10, 100)
            self.update_speed()
